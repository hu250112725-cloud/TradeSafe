...
function checkLegality({ species, level, isShiny, ivs }) {
  ...
  if (level != null && level !== "" && !(lv >= 1 && lv <= 100)) reasons.push("Nivel fuera de rango (1–100)");
  const tieneIvs = Array.isArray(ivs) && ivs.length === 6 && ivs.every((v) => Number.isFinite(v));
  if (tieneIvs && ivs.some((v) => !(v >= 0 && v <= 31))) reasons.push("IVs fuera de rango (0–31)");
  if (isShiny && SHINY_LOCKED.includes(sp))
    reasons.push(`${species} tiene el shiny bloqueado en todos los juegos`);
  if (MIN_LEVEL[sp] && lv && lv < MIN_LEVEL[sp])
    reasons.push(`${species} no puede existir por debajo del nivel ${MIN_LEVEL[sp]}`);
  const warnings = [];
  if (THREE_PERFECT.includes(sp) && tieneIvs && ivs.filter((v) => v === 31).length < 3)
