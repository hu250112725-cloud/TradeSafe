...
// Interruptor: el correo solo se activa con MAIL_ENABLED=true en Vercel
// (pensado para cuando haya un dominio propio verificado en Resend).
export const mailActivo = () => process.env.MAIL_ENABLED === "true";

export async function sendMail(to, subject, html) {
  if (!mailActivo()) return { ok: false, reason: "desactivado" };
  ...
  try {
    ...
    if (!r.ok) {
      let detalle = "";
      try { detalle = (await r.text()).slice(0, 300); } catch { /* sin cuerpo */ }
      return { ok: false, reason: "envio-" + r.status + " " + detalle };
    }
    return { ok: true };
  } catch {
