...
/* ---------- Revisión de una captura de prueba ---------- */
const INSTRUCCIONES_PRUEBA = `Eres un revisor de pruebas de intercambios de Pokémon HOME.
Te doy una captura y el código del intercambio. Debes comprobar SOLO lo que se vea en la imagen.
Responde ÚNICAMENTE con un objeto JSON, sin texto alrededor:
{"esPokemon":boolean,"codigoVisible":boolean,"especieVisible":string|null,
 "shiny":boolean|null,"sospechas":string[],"resumen":string}
Reglas:
- "esPokemon": true si es una captura de Pokémon HOME o de un juego de Pokémon.
- "codigoVisible": true SOLO si el código exacto que te doy aparece escrito en la imagen
  (por ejemplo como nombre de una caja o en el perfil del entrenador). Si no lo ves, false.
- "especieVisible": el Pokémon principal que se vea, o null.
- "sospechas": lista breve de cosas raras que observes (imagen recortada de otra foto, texto superpuesto,
  fecha o interfaz que no cuadra, captura de una captura, marcas de edición). Vacía si no ves nada raro.
- "resumen": una frase corta y neutra describiendo lo que se ve.
No acuses a nadie. Limítate a describir lo observable.`;

export async function revisarPrueba(imagenBase64, { codigo, especie }) {
  const txt = await llamar({
    model: MODELO_VISION,
    temperature: 0,
    max_tokens: 400,
    response_format: { type: "json_object" },
    messages: [
      { role: "system", content: INSTRUCCIONES_PRUEBA },
      {
        role: "user",
        content: [
          { type: "text", text: `Código del intercambio: ${codigo}. Pokémon acordado: ${especie || "desconocido"}.` },
          { type: "image_url", image_url: { url: imagenBase64 } },
        ],
      },
    ],
  ...
  return {
    esPokemon: !!d.esPokemon,
    codigoVisible: !!d.codigoVisible,
    especieVisible: d.especieVisible ? String(d.especieVisible).slice(0, 40) : null,
    shiny: typeof d.shiny === "boolean" ? d.shiny : null,
    sospechas: (Array.isArray(d.sospechas) ? d.sospechas : []).slice(0, 4).map((x) => String(x).slice(0, 120)),
    resumen: String(d.resumen || "").slice(0, 200),
  };
}
