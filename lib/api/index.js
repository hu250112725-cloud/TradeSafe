...
async function createUser({ name, trainer, email, pass }, role, verified) {
  ...
  const r = await q(
    `INSERT INTO users (email, pass_hash, display_name, trainer_name, role, verified)
     VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
    [email.trim().toLowerCase(), hash, name.trim(), trainer.trim(), role, verified]
  );
  return r.rows[0];
}
...
app.get("/api/state", auth, async (req, res) => {
  ...
  res.json({
    me: { id: me.id, displayName: me.display_name, trainerName: me.trainer_name, role: me.role, verified: me.verified, createdAt: me.created_at, email: me.email },
...
app.post("/api/users/:id/verify", auth, staff, async (req, res) => {
  await q(`UPDATE users SET verified=true WHERE id=$1`, [req.params.id]);
