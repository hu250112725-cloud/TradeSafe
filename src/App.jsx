...
function AuthScreen({ refresh, hasUsers }) {
  ...
  const submit = () => run(async () => {
    ...
    const d = { name: f.name, trainer: f.trainer, email: f.email, pass: f.pass };
    ...
    <div>
      ...
      {t.state === "pre_proof" && (
        <div className="mt-14">
          ...
          {!yoProbe && <button className="btn mt-14" disabled={busy} onClick={() => act("proof")}>📷 Subir captura con el código</button>}
        </div>
      )}
      ...
      {t.state === "post_proof" && (
        <div className="mt-14">
          ...
          {!yoConfirme ? (
            <button className="btn mt-14" disabled={busy} onClick={() => act("confirm")}>📷 Subir captura final y confirmar</button>
          ) : (
...
function Perfil({ me, refresh }) {
  ...
  return (
    <div>
      ...
      {!me.verified && (
        <div className="mt-14">
          <Aviso tipo="oro">Un moderador revisará tu vinculación con HOME (código temporal + captura). Mientras tanto puedes navegar, pero tu perfil aparece "sin verificar".</Aviso>
        </div>
      )}
      <div className="ficha mt-14">
      ...
      {pane === "verif" && (pendVerif.length === 0 ? <Vacio icono="🪪">No hay verificaciones pendientes.</Vacio> :
        pendVerif.map((u) => (
          <div key={u.id} className="ficha" style={{ marginBottom: 14 }}>
            ...
            <p className="txt-xs suave mt-6">Comprueba que la captura de su perfil de HOME muestra el código temporal generado por la app.</p>
